...
class FlightRepository @Inject() (dbConfigProvider: DatabaseConfigProvider)(implicit ec: ExecutionContext) {
   ...
   */
  private class FlightsTable(tag: Tag) extends Table[Flights](tag, "flights") {
     ...
     */
    def * = (id, source, destination, carrier, dateOfTravel, dateOfPriceFall, predictedPrice, latitude,longitude, distance,seats) <> ((Flights.apply _).tupled, Flights.unapply)
  }
...
*/
  //def searchFlights(s:String, d:String, date:String): Future[Flight] = db.run(flights.filter(_.source==s ).filter(_.destination==d).filter(_.dateOfTravel==d)).result
    //def searchFlights(S:String, d:String, date:String):Future[Flight] = {}
def create(source: String, destination: String, carrier: String, dateOfTravel: String, dateOfPriceFall:String, predictedPrice: Int, latitude: String, longitude: String, distance: String, seats: Int): Future[Flights] = db.run {
    // We create a projection of just the name and age columns, since we're not inserting a value for the id column
    (flights.map(p => (p.source, p.destination,p.carrier,p.dateOfTravel,p.dateOfPriceFall,p.predictedPrice,p.latitude,p.longitude,p.distance,p.seats))
      // Now define it to return the id, because we want to know what id was generated for the person
      returning flights.map(_.id)
      // And we define a transformation for the returned value, which combines our original parameters with the
      // returned id
      into ((details, id) => Flights(id, details._1, details._2, details._3, details._4, details._5, details._6, details._7, details._8, details._9))
    // And finally, insert the person into the database
   ...
   */
  def list(): Future[Seq[Flights]] = db.run {
    flights.result
  }
}
