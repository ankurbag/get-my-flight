...
class FlightControllerSpec extends PlaySpecification with Mockito with Results {

  implicit val mockedRepo: FlightRepository = mock[FlightRepository]


  "FlightController" should {
...
class WithFltApplication(implicit mockedRepo: FlightRepository) extends WithApplication {
  val messageAPI = new DefaultMessagesApi(Environment.simple(), app.configuration, new DefaultLangs(app.configuration))
  val flightController = new FlightController(mockedRepo, messageAPI)
}
